...
spacing_supercell_nbands_list = {
    ...
    "C60": {"spacing": (0.2,), "supercell": ([1, 1, 1],), "nbands": (150,)},
    "ATP": {"spacing": (0.2,), "supercell": ([1, 1, 1],), "nbands": (100,)},
    "GSH": {"spacing": (0.2,), "supercell": ([1, 1, 1],), "nbands": (70,)},
    "aspirin": {"spacing": (0.2,), "supercell": ([1, 1, 1],), "nbands": (40,)},
}
        ...
        for material, (upf_files, pp_type) in materials_pseudopotentials.items():
            ...
            if precond == "gapp":
                summary_file = os.path.join(
                    output_dir, f"results_summary_gapp_{material}.txt"
                )

            elif precond == "poisson":
                summary_file = os.path.join(
                    output_dir, f"results_summary_poisson_{material}.txt"
                )

            elif precond == "shift-and-invert" and innerprecond == "gapp":
                summary_file = os.path.join(
                    output_dir,
                    f"results_summary_ISI_gapp_pcg_{gapp_pcg_num}_{material}.txt",
                )

            elif precond == "shift-and-invert" and innerprecond == "Neumann":
                summary_file = os.path.join(
                    output_dir,
                    f"results_summary_shift_Neumann_pcg_{Neumann_pcg_num}_{material}.txt",
                )
            elif precond == "Neumann":
                summary_file = os.path.join(
                    output_dir, f"results_summary_Neumann_{material}.txt"
                )
            else:
                raise NotImplementedError(
                    f"Unknown precond: {precond}, inner: {innerprecond}"
                )
            ...
            if not os.path.exists(summary_file):
                with open(summary_file, "w") as f:
                    if precond == "Neumann":
                        f.write(
                            "Spacing, Preconditioner, Material, NBands, Supercell, Median Elapsed Time (s), Max Iterations, Total Precond Time, Per Precond Time, Solver Type, Order, PCG_Number\n"
                        )

                    elif precond == "shift-and-invert" and innerprecond == "Neumann":
                        f.write(
                            "Spacing, Preconditioner, InnerPreconditioner, Material, NBands, Supercell, Median Elapsed Time (s), Max Iterations, Total Precond Time, Per Precond Time, Solver Type, Order, PCG_Number\n"
                        )

                    elif precond == "shift-and-invert" and innerprecond == "gapp":
                        f.write(
                            "Spacing, Preconditioner, InnerPreconditioner, Material, NBands, Supercell, Median Elapsed Time (s), Max Iterations, Total Precond Time, Per Precond Time, Solver Type, PCG_Number\n"
                        )

                    else:
                        f.write(
                            "Spacing, Preconditioner, Material, NBands, Supercell, Median Elapsed Time (s), Max Iterations, Total Precond Time, Per Precond Time, Solver Type,\n"
                        )
            ...
            supercell_factors_list = spacing_supercell_nbands_list[material][
                "supercell"
            ]
            nbands_list = spacing_supercell_nbands_list[material]["nbands"]

            base_combos = itertools.product(
                nbands_list, spacing_list, (precond,), supercell_factors_list
            )
            if precond == "Neumann":
                combos = (
                    (
                    ...
                    for nbands, spacing, precond, supercell, outerorder, error_cutoff in itertools.product(
                        nbands_list,
                        spacing_list,
                        (precond,),
                        supercell_factors_list,
                        outerorder_list,
                        error_cutoff_list,
                    )
                )
                ...
                combos = (
                    (nbands, spacing, precond, supercell, None, None, None, None)
                    for nbands, spacing, precond, supercell in base_combos
                )
                gapp_pcg_num = None
                Neumann_pcg_num = None
                ...
                combos = (
                    (nbands, spacing, precond, supercell, None, None, "gapp", None)
                    for nbands, spacing, precond, supercell in base_combos
                )
                gapp_pcg_num = 5
                Neumann_pcg_num = None
                ...
                combos = (
                    ...
                    for nbands, spacing, precond, supercell, innerorder in itertools.product(
                        nbands_list,
                        spacing_list,
                        (precond,),
                        supercell_factors_list,
            ...
            for (
                ...
                for idx in range(1):
                    ...
                    add_opt(
                        command, "--nbands", int(nbands * np.prod(supercell_factors))
                    )
                    ...
                    # scf  iter
                    match = re.search(
                        r"Elapsed time\[davidson\]\s*:\s*([\d.]+)\s*sec", stdout
                    )
                    if match:
                ...
                if diagonal_total_times:
                    ...
                    print(f"Updated labeled_results: {labeled_results}")  # for debug
                    ...
                    max_iterations = (
                        max(max_diagonal_iterations_list)
                        if max_diagonal_iterations_list
                        else 0
                    )

                    max_scf_iterations = (
                        np.median(max_scf_iteration_list)
                        if max_scf_iteration_list
                        else 0
                    )
