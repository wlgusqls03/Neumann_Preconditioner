...
def _random_rotate(mol: Atoms, enable: bool, seed: Optional[int] = None) -> Atoms:
    ...
    for ang, ax in zip((rng.uniform(0, 360), rng.uniform(0, 360), rng.uniform(0, 360)), ("x", "y", "z")):
        m.rotate(ang, v=ax, center="COM", rotate_cell=False)
    return m

def _place_on_positions(mol: Atoms, offsets: Iterable[Tuple[float, float, float]]) -> Atoms:
    out = None
    for off in offsets:
        m = mol.copy()
        m.translate(off)
...
def _grid_offsets(nx: int, ny: int, sx: float, sy: float) -> List[Tuple[float, float, float]]:
    return [(ix * sx, iy * sy, 0.0) for iy in range(ny) for ix in range(nx)]

def _lattice_offsets(nx: int, ny: int, nz: int, sx: float, sy: float, sz: float) -> List[Tuple[float, float, float]]:
    return [(ix * sx, iy * sy, iz * sz) for iz in range(nz) for iy in range(ny) for ix in range(nx)]

def _compact_cell_from_positions(centers: List[Tuple[float, float, float]], wall_gap: float, rectangular: bool) -> Tuple[float, float, float]:
    if not centers:
...
def build_c60_cluster(
    ...
    if layout == "line":
        ...
        if nx * ny * nz != n_molecules:
            raise ValueError(f"lattice_shape({nx}x{ny}x{nz}) != n_molecules({n_molecules})")
...
#    print(build_c60_cluster(            
#        input_xyz="C60.xyz",                
#        n_molecules=8,                      # C60의 개수
#        layout="lattice",                   # 3차원으로 배치
#        lattice_shape=(2, 2, 2),            # 각 축당 2개씩 배치
#        spacings_xyz=(12.0, 12.0, 12.0),    # 분자 사이의 간격
#        wall_gap=5.0,                       # 벽과 원자사이의 간격
#        rectangular=True,
